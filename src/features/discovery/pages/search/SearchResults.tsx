...
const buildFiltersFromParams = (params: URLSearchParams) => {
  const minExperienceYears = readNumberParam(
    params.get("minExperienceYears"),
    0,
  );
  const maxExperienceYears = readNumberParam(
    params.get("maxExperienceYears"),
    0,
  );
  const minYearsOfOperation = readNumberParam(
    params.get("minYearsOfOperation"),
    0,
  );
  const maxYearsOfOperation = readNumberParam(
...
const buildQueryParams = (filters: {
  ...
  if (filters.maxExperienceYears > 0)
    params.set("maxExperienceYears", String(filters.maxExperienceYears));
  if (filters.minYearsOfOperation > 0)
  ...
  if (filters.minProjects > 0)
    params.set("minProjects", String(filters.minProjects));
...
const normalizeResult = (item: PublicSearchProfileItem) => {
  ...
  const specialization = isEnterprise
    ? item.sector || "Entreprise"
    : item.specialization || item.sector || "Professionnel";
  ...
  return {
    ...
    photo:
      item.avatarUrl ||
      `https://api.dicebear.com/7.x/identicon/svg?seed=${item.userId}`,
    tags,
...
const normalizeDraftComparable = (
  filters: ReturnType<typeof buildFiltersFromParams>,
) => ({
...
function SearchResults() {
  ...
  const [locationResolved, setLocationResolved] = useState(() =>
    Boolean(
      searchParams.get("country")?.trim() && searchParams.get("city")?.trim(),
    ),
  );
  const authShortcutLabel = isAuthenticated ? "Dashboard" : "Connexion";
  const authShortcutRoute = isAuthenticated
    ? getDashboardRoute()
    : "/connexion";
  ...
  useEffect(() => {
    setProximityEnabled(
      typeof formFilters.lat === "number" &&
        typeof formFilters.lng === "number",
    );
    ...
    () =>
      [
        activeFilters.search
          ? {
              key: "search" as const,
              label: `Recherche: ${activeFilters.search}`,
            }
          : null,
        activeFilters.secteur
          ? {
              key: "secteur" as const,
              label: `Secteur: ${activeFilters.secteur}`,
            }
          : null,
        activeFilters.pays
          ? { key: "pays" as const, label: `Pays: ${activeFilters.pays}` }
          : null,
        activeFilters.ville
          ? { key: "ville" as const, label: `Ville: ${activeFilters.ville}` }
          : null,
        activeFilters.type
          ? {
              key: "type" as const,
        ...
        activeFilters.minRating > 0
          ? {
              ...
              label: `Note min: ${activeFilters.minRating}`,
            }
          : null,
        activeFilters.verifiedOnly
          ? { key: "verifiedOnly" as const, label: "Verifies uniquement" }
          : null,
        activeFilters.premiumOnly
          ? { key: "premiumOnly" as const, label: "Premium uniquement" }
          : null,
        activeFilters.minProjects > 0
          ? {
              key: "minProjects" as const,
              label: `Projets min: ${activeFilters.minProjects}`,
            }
          : null,
        activeFilters.maxHourlyRate > 0
          ? {
              key: "maxHourlyRate" as const,
              label: `Tarif: ${activeFilters.maxHourlyRate}`,
            }
          : null,
        typeof activeFilters.lat === "number" &&
        typeof activeFilters.lng === "number"
          ? {
              key: "lat" as const,
        ...
        (activeFilters.maxExperienceYears || activeFilters.minExperienceYears) >
          0
          ? {
  ...
  useEffect(() => {
    const hasLocationInParams = Boolean(
      activeFilters.pays.trim() || activeFilters.ville.trim(),
    );
  ...
  const requestPayload = useMemo(() => {
    ...
    return {
      ...
      minProjects:
        activeFilters.minProjects > 0 ? activeFilters.minProjects : undefined,
      maxHourlyRate:
        activeFilters.maxHourlyRate > 0
          ? activeFilters.maxHourlyRate
          : undefined,
  ...
  const shouldWaitForLocation =
    !locationResolved && !activeFilters.pays && !activeFilters.ville;
  ...
  const pros = useMemo(
    () => searchResponse?.data?.pros?.content ?? [],
    [searchResponse],
  );
  ...
  const secteurs = useMemo(
    () => [
      "Tous les secteurs",
      ...sortByLabel(
        (professionSectorMap as SectorMapEntry[]).map((entry) => entry.sector),
      ),
    ],
    [],
  );
  ...
  const normalizedEnterprises = useMemo(
    () => enterprises.map(normalizeResult),
    [enterprises],
  );
  ...
  const filteredResults = useMemo(() => {
    ...
    return byType.filter((item) => {
      if (activeFilters.minRating > 0 && item.note < activeFilters.minRating)
        return false;
      if (activeFilters.verifiedOnly && !item.verified) return false;
      if (activeFilters.premiumOnly && !item.premium) return false;
      if (
        activeFilters.minProjects > 0 &&
        item.projetsCollaboration < activeFilters.minProjects
      )
        return false;

      if (activeFilters.maxHourlyRate > 0) {
        const rate =
          typeof item.hourlyRate === "number" ? item.hourlyRate : null;
    ...
    <div className="search-page">
      ...
      <div className={`search-sidebar ${menuOpen ? "open" : ""}`}>
        ...
        <nav className="search-sidebar-nav">
          ...
          <button
            className="search-connexion-btn"
            onClick={() => {
              goToAuthShortcut();
              closeMenu();
            }}
      ...
      <main className="search-main">
        ...
        <section className="search-filters-section">
          <form className="search-filters-bar" onSubmit={handleSearch}>
            ...
            <div className="search-filter-item">
              <input
                ...
                placeholder={
                  formFilters.pays ? "Ville (choisir ou saisir)" : "Ville"
                }
                value={formFilters.ville}
                onChange={(e) =>
                  setFormFilters((prev) => ({ ...prev, ville: e.target.value }))
                }
              />
            ...
            <div className="search-advanced-filters">
              ...
              <div className="search-advanced-layout">
                <div className="search-advanced-left">
                  <div
                    className={`search-filter-item search-proximity-card ${proximityEnabled ? "active" : ""}`}
                  >
                    <div className="search-proximity-card-header">
                      <FiMapPin className="search-proximity-icon" />
                      <label
                        className="search-proximity-label"
                        htmlFor="search-proximity-checkbox"
                      >
                        <span>Proximite</span>
                      </label>
                      <label
                        className="search-proximity-checkline"
                        htmlFor="search-proximity-checkbox"
                      >
                        <input
                    ...
                    typeof formFilters.lng === "number" ? (
                      <>
                        <div className="search-proximity-status active">
                          Localisation detectee
                        </div>
                        <div className="search-proximity-coords">
                          Lat: {formFilters.lat.toFixed(5)} | Lng:{" "}
                          {formFilters.lng.toFixed(5)}
                        </div>
                      </>
                    ) : (
                      <div
                        className={`search-proximity-status ${geolocationLoading ? "loading" : ""}`}
                      >
                        {geolocationLoading
                          ? "Detection en cours..."
                ...
                <div className="search-advanced-right">
                  ...
                  <div className="search-filter-item">
                    <label htmlFor="minProjects">
                      Nombre de projet: {formFilters.minProjects}
                    </label>
                    <input
                      id="minProjects"
                      type="range"
                      ...
                      onChange={(e) =>
                        setFormFilters((prev) => ({
                          ...prev,
                          minProjects: Number(e.target.value),
                        }))
                      }
                    />
                  ...
                  {(formFilters.type === "freelance") && (
                    <div className="search-filter-item">
                      <label htmlFor="maxExperienceYears">
                        Experience:{" "}
                        {formFilters.maxExperienceYears ||
                          formFilters.minExperienceYears}{" "}
                        ans
                      </label>
                      <input
                  ...
                  {(formFilters.type === "entreprise") && (
                    <div className="search-filter-item">
                      <label htmlFor="maxYearsOfOperation">
                        Exercice:{" "}
                        {formFilters.maxYearsOfOperation ||
                          formFilters.minYearsOfOperation}{" "}
                        ans
                      </label>
                      <input
                        ...
                        onChange={(e) =>
                          setFormFilters((prev) => ({
                            ...prev,
                            minYearsOfOperation: Number(e.target.value),
          ...
          {/* Filtres rapides */}
          <div className="search-quick-filters" style={{ marginTop: "1rem" }}>
            <button
              type="button"
